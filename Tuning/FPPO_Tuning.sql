...
PROCEDURE CREATELINK_FOR_POT(SS IN out rmon.fcscriptstate);
...
PROCEDURE plsql_upd_apnx(p_SS IN OUT RMON.fcScriptState,
    nTREE_ID  IN NUMBER,
    sREPORT_NAME IN VARCHAR2,
    nMODE in NUMBER default 0
    );
/*Откат процедуры plsql_upd_apnx*/
PROCEDURE plsql_rolb_upd_apnx(p_SS IN OUT RMON.fcScriptState);
...
sACCT VARCHAR2,         --Параметр указывающий какой тип счета получить. Счета берутся из договоров об коротношениях
                        ...
                        --                                                       S - доп. объект
nPACGN N67.N67ACGN%TYPE,--Код группового имени Пассивного счета
nAACGN N67.N67ACGN%TYPE --Код группового имени Активного счета
...
PROCEDURE update_n38tssd(p_SS IN OUT RMON.fcScriptState);
...
PROCEDURE SET_B61CBNC(P_SS IN OUT RMON.FCSCRIPTSTATE,AGID N30.N30AGID%TYPE,CMPC B61.B61CMPC%TYPE);
...
--$Author GPolyakov
--Процедура обеспечивает обновление
PROCEDURE SET_B61CBNC(P_SS IN OUT RMON.FCSCRIPTSTATE,AGID N30.N30AGID%TYPE,CMPC B61.B61CMPC%TYPE)
IS
R1 N30%ROWTYPE;
R2 B61%ROWTYPE;
BEGIN
    SELECT * INTO R1 FROM N30 WHERE N30AGID=AGID;
    BEGIN
      SELECT * INTO R2 FROM B61 WHERE B61CMPC=CMPC;
      IF (P_SS.M_BROLLBACK = 1) THEN
        IF (R2.B61CBNC)=TO_CHAR(R1.N30CLID) THEN
         UPDATE B61 SET B61CBNC=NULL WHERE B61.B61CMPC=CMPC;
        END IF;
      ELSE
        UPDATE B61 SET B61CBNC=TO_CHAR(R1.N30CLID) WHERE B61.B61CMPC=CMPC;
      END IF;
    EXCEPTION
      WHEN NO_DATA_FOUND THEN
        P_SS.M_SERROR := 'L1220'; --Не найдено подразделение ^
        p_SS.m_sErrorParams := CMPC;
    END;
    COMMIT;
EXCEPTION
  WHEN NO_DATA_FOUND THEN
    P_SS.M_SERROR := 'L1305'; --Не найден договор или код организации. AGID=^
    p_SS.m_sErrorParams := AGID;
END;
